...
function createVisit(item: Visit, template: HTMLTemplateElement, depth: number) {
...
function createJourney(journey: Visit, template: HTMLTemplateElement, depth: number) {
...
async function constructHistory(historyItems: chrome.history.HistoryItem[]) {
    ...
    console.log(urlTree)
    const template: HTMLTemplateElement = <HTMLTemplateElement>document.getElementById('historyTemplate')!;
...
document.getElementById('searchSubmit')!.onclick = async function () {
    ...
    const searchQuery = (<HTMLInputElement>document.getElementById('searchInput')!).value;
